...
pub fn make_key_management_route(
    ...
    Resource::build("/biome/users/{user_id}/keys").add_method(Method::Post, move |request, payload| {
        ...
        let user_id = request.match_info().get("user_id").unwrap_or_default().to_owned();
        let secret = match secret_manager.secret() {
            Ok(secret) => secret,
            Err(err) => {
                debug!("Failed to fetch secret {}", err);
                return  Box::new(HttpResponse::Unauthorized().json(ErrorResponse::internal_error()).into_future())
            }
        };
        ...
        if let Err(err) = validate_token(&auth_token, &secret, &rest_config.issuer(), |claim| {
            if user_id != claim.user_id() {
                return Err(TokenValidationError::InvalidClaim(format!("User is not update keys for user {}", user_id)));
            }
            Ok(())
        }) {
            debug!("Invalid token: {}", err);
